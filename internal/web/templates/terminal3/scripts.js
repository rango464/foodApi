...
class TradingTerminal {
    ...
    initializeElements() {
        this.elements = {
            ...
            tabNavLeft: document.getElementById('tabNavLeft'),
            tabNavRight: document.getElementById('tabNavRight'),
    ...
    setupTabNavigation() {
        ...
        this.elements.tabNavLeft.addEventListener('click', () => {
            this.scrollTabs('left');
        });

        this.elements.tabNavRight.addEventListener('click', () => {
            this.scrollTabs('right');
        });

        // Обработчик прокрутки для обновления состояния кнопок
        this.elements.pairTabs.addEventListener('scroll', () => {
            this.updateTabNavButtons();
        });

        // Обновляем состояние кнопок при изменении размера
        window.addEventListener('resize', () => {
            setTimeout(() => this.updateTabNavButtons(), 100);
        });
    }

    // Прокрутка вкладок
    scrollTabs(direction) {
        const tabsContainer = this.elements.pairTabs;
    ...
    updateTabNavButtons() {
        const tabsContainer = this.elements.pairTabs;
        const leftBtn = this.elements.tabNavLeft;
        const rightBtn = this.elements.tabNavRight;
    ...
    scrollToActiveTab() {
        const activeTab = this.elements.pairTabs.querySelector('.pair-tab.active');
        if (activeTab) {
            const tabsContainer = this.elements.pairTabs;
            const tabRect = activeTab.getBoundingClientRect();
            const containerRect = tabsContainer.getBoundingClientRect();
            
            // Если вкладка не видна полностью, прокручиваем к ней
    ...
    updatePairTabs() {
        ...
        // Обновляем состояние кнопок навигации после создания вкладок
        setTimeout(() => {
            this.updateTabNavButtons();
    ...
    switchToPair(pair) {
        ...
        // Обновляем активную вкладку без пересоздания всех вкладок
        this.elements.pairTabs.querySelectorAll('.pair-tab').forEach(tab => {
            tab.classList.remove('active');
            if (tab.textContent === this.formatPairName(pair)) {
