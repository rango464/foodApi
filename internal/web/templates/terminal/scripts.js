...
class TradingTerminal {
    ...
    createPairCheckboxes() {
        ...
        this.availablePairs.forEach(pair => {
            ...
            checkbox.innerHTML = `
                <input type="checkbox" id="pair-${pair}" ${this.selectedPairs.has(pair) ? 'checked' : ''}>
                <label for="pair-${pair}">${pair.replace('USDT', '/USDT').replace('BTC', '/BTC')}</label>
            `;
    ...
    updatePairTabs() {
        ...
        Array.from(this.selectedPairs).forEach(pair => {
            ...
            tab.textContent = pair.replace('USDT', '/USDT').replace('BTC', '/BTC');
    ...
    showChartTooltip(x, y) {
        ...
        this.elements.chartTooltip.innerHTML = `
            <div><strong>${this.currentPair.replace('USDT', '/USDT').replace('BTC', '/BTC')}</strong></div>
    ...
    openTradeModal(e) {
        ...
        Array.from(this.selectedPairs).forEach(pair => {
            ...
            option.value = pair;
            option.textContent = pair.replace('USDT', '/USDT').replace('BTC', '/BTC');
        ...
        this.trades.forEach(trade => {
            ...
            tradeElement.innerHTML = `
                <div class="trade-header">
                    <span>${trade.pair.replace('USDT', '/USDT').replace('BTC', '/BTC')}</span>
    ...
    updateOrderBookUI() {
        ...
        this.elements.orderBook.querySelector('h3').textContent = 
            `Биржевой стакан - ${this.currentPair.replace('USDT', '/USDT').replace('BTC', '/BTC')}`;
